...
MongoDB.prototype.create = function(model, data, options, callback) {
  ...
  this.execute(model, 'insert', data, { safe: true }, patchWithContext(function(err, result) {
    if (self.debug) {
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  if (fields) {
    ...
    this.execute(model, 'find', query, findOpts, processResponse.bind(this));
  } else {
    this.execute(model, 'find', query, processResponse.bind(this));
  }
