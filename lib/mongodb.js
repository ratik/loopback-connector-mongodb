...
MongoDB.prototype.execute = function(model, command) {
  ...
  const doExecute = () => {
    ...
    self.notifyObserversAround('execute', context, (context, done) => {
      args[args.length - 1] = function(err, result) {
        if (err) {
          debug('Error: ', err);
        } else {
          context.res = result;
          debug('Result: ', 'ok'/*result/**/);
        }
        done(err, result);
      };
...
MongoDB.prototype.create = function(model, data, options, callback) {
  ...
  this.execute(model, 'insert', data, { safe: true }, (err, result) => {
    if (self.debug) {
...
MongoDB.prototype.save = function(model, data, options, callback) {
  ...
  this.execute(model, 'save', data, {}, (err, result) => {
    if (!err) {
...
MongoDB.prototype.exists = function(model, id, options, callback) {
  ...
  this.execute(model, 'findOne', { _id: id }, (err, data) => {
    if (self.debug) {
...
MongoDB.prototype.find = function find(model, id, options, callback) {
  ...
  this.execute(model, 'findOne', { _id: oid }, (err, data) => {
    if (self.debug) {
...
MongoDB.prototype.destroy = function destroy(model, id, options, callback) {
  ...
  id = self.coerceId(model, id);
  this.execute(model, 'remove', { _id: id }, (err, result) => {
    if (self.debug) {
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  const processResponse = async (err, cursor) => {
    if (err) {
      return callback(err);
    }
...
MongoDB.prototype.destroyAll = function destroyAll(model, where, options, callback) {
  ...
  this.execute(model, 'remove', where || {}, (err, info) => {
...
MongoDB.prototype.count = function count(model, where, options, callback) {
  ...
  where = self.buildWhere(model, where);
  this.execute(model, 'count', where, (err, count) => {
    if (self.debug) {
...
MongoDB.prototype.replaceWithOptions = function(model, id, data, options, cb) {
  ...
  var idName = self.idName(model);
  delete data[idName];
  this.execute(model, 'update', { _id: id }, data, options, (err, info) => {
